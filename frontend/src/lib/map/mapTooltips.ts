import type { MapLayerID, MapLayerSource, MapSourceLayer } from './mapLayers';
...
export const targetFeatureFieldsMap: Partial<
    Record<MapSourceLayer, MapHoverSection>
> = {
    'tx_eco_l4-1joalj': {
    ...
    'observations-circles': {
        ...
        fields: [
            ...
            { label: 'Institution', property: 'institution_code' },
            {
                ...
                property: 'gbif_id',
                transform: (v: string) => {
                    if (isValidURL(`https://www.gbif.org/occurrence/${v}`)) {
                        return `
                            <a 
...
export function buildTooltipSections(
    ...
    for (const feature of features) {
        const sourceID = feature.source as MapLayerID | undefined;
        if (!sourceID) continue;

        const sourceLayer = feature.sourceLayer as MapSourceLayer;
