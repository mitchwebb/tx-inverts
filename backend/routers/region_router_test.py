...
class TestGetRegionInfo():
    ...
    async def test_basic_region_info_retrieval(self, client, basic_regions_view):
        response = await client.get('/region/get_region_info', params={'region_id': 'bf8131cd-ebc8-41c1-b17f-766eec7e48fc'})
        result = response.json()

        assert result['name'] == 'Travis'
        assert result['region_type'] == 'county'

        response = await client.get('/region/get_region_info', params={'region_id': '5dd66bb5-f81c-483d-b289-81a84174ec3a'})
        result = response.json()
    ...
    async def test_404_missing_region(self, client, basic_regions_view):
        response = await client.get('/region/get_region_info', params={'region_id': 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa'})
...
class TestSearchRegions():
    ...
    async def test_basic_counties_search(self, client, basic_regions_view):
        response = await client.get('/region/search_counties', params={'search_term': 'tra'})
        ...
        response = await client.get('/region/search_counties', params={'search_term': 'den'})
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 1
    ...
    async def test_basic_counties_empty(self, client, basic_regions_view):
        response = await client.get('/region/search_counties', params={'search_term': 'zzz'})
        assert response.status_code == 200
        result = response.json()
    ...
    async def test_basic_parks_search(self, client, basic_regions_view):
        response = await client.get('/region/search_parks', params={'search_term': 'spa'})
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 1
        ...
        response = await client.get('/region/search_parks', params={'search_term': 'bal'})
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 1
    ...
    async def test_basic_parks_empty(self, client, basic_regions_view):
        response = await client.get('/region/search_parks', params={'search_term': 'zzz'})
        assert response.status_code == 200
        result = response.json()
        assert not result  # Results array should be empty
