...
async def refresh_materialized_view(
	...
	async with conn.cursor() as cur:
		if not view_exists:
			view_def = MATERIALIZED_VIEWS.get(view_name)
			print(f'Materialized view {view_name} not yet created, creating now...')
			await cur.execute(view_def['create_sql'])
		else:
			print(f'Materialized view {view_name} already created, refreshing...')
			await cur.execute(
       			sql.SQL('REFRESH MATERIALIZED VIEW {}').format(sql.Identifier(view_name))
          	)
